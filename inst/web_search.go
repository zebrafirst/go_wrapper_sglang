...
var (
	withIndexPrompt = `# 以下内容是基于用户发送的消息的搜索结果:
	{{.SearchResults}}
	...
	# 用户消息为：
	{{.Question}}`
	withoutIndexPrompt = `# 以下内容是基于用户发送的消息的搜索结果:
	{{.SearchResults}}
...
var (
	withIndexPromptTmpl    *template.Template
	withoutIndexPromptTmpl *template.Template
)

func InitWebSearchTemplate() {
	if config.GetBaseConf().PromptSearchTemplate != "" {
		withIndexPromptTmpl = template.Must(template.New("t1").Parse(config.GetBaseConf().PromptSearchTemplate))
	} else {
		withIndexPromptTmpl = template.Must(template.New("t1").Parse(withIndexPrompt))
	}

	if config.GetBaseConf().PromptSearchTemplateNoIndex != "" {
		withoutIndexPromptTmpl = template.Must(template.New("t2").Parse(config.GetBaseConf().PromptSearchTemplateNoIndex))
	} else {
		withoutIndexPromptTmpl = template.Must(template.New("t2").Parse(withoutIndexPrompt))
	}
}
...
func NewWebSearch(ldMsg *LoaderMessage) *WebSearch {
	...
	for _, v := range ldMsg.Messages {
		if v.Role == common.ROLE_TOOL && v.ToolCallID == common.TOOL_CALL_ID_IFLY_SEARCH {
			...
			webSearch.Msg = v
			webSearch.WithRefLabel = v.ShowRefLabel
		}
	}
	return webSearch
}
...
func (ws *WebSearch) BuildWebSearchPrompt(question string) string {
	...
	if ws.WithRefLabel {
		tmpl = withIndexPromptTmpl
	} else {
		tmpl = withoutIndexPromptTmpl
	}
	...
	if err != nil {
		th3apiutils.WLogger.Warn("Search prompt build failed", zap.Any("err", err))
	}
	return sb.String()
}
