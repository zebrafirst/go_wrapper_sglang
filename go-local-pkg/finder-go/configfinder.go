...
import (
	...
	common "git.iflytek.com/AIaaS/finder-go-self/common"
	errors "git.iflytek.com/AIaaS/finder-go-self/errors"
	"git.iflytek.com/AIaaS/finder-go-self/log"
	"git.iflytek.com/AIaaS/finder-go-self/storage"
	"git.iflytek.com/AIaaS/finder-go-self/utils/fileutil"
)
...
func NewConfigFinder(root string, bc *common.BootConfig, sm storage.StorageManager) *ConfigFinder {
	finder := &ConfigFinder{
		locker:          sync.Mutex{},
		rootPath:        root,
		shouldExpandEnv: false,
		config:          bc,
		storageMgr:      sm,
		usedConfig:      sync.Map{},
	}

	if arg := strings.ToLower(os.Getenv("FINDER_EXPAND_ENV")); arg != "" {
		if v, err := strconv.ParseBool(arg); err == nil {
			finder.shouldExpandEnv = v
		} else if v, err := strconv.ParseInt(arg, 10, 64); err == nil {
			finder.shouldExpandEnv = (v != 0)
		}
	}
...
type ConfigOptionFunc func(map[string]*common.Config) map[string]*common.Config

func ExpandEnv() ConfigOptionFunc {
	return func(m map[string]*common.Config) map[string]*common.Config {
		for _, cfg := range m {
			cfg.File = []byte(os.ExpandEnv(string(cfg.File)))
		}
		return m
	}
}

func Expand(mapping map[string]string) ConfigOptionFunc {
	return func(m map[string]*common.Config) map[string]*common.Config {
		for _, cfg := range m {
			cfg.File = []byte(os.Expand(string(cfg.File), func(s string) string {
				return mapping[s]
			}))
		}
		return m
...
func (f *ConfigFinder) postProcess(cfgs map[string]*common.Config, opts ...ConfigOptionFunc) map[string]*common.Config {
	if f.shouldExpandEnv {
		cfgs = ExpandEnv()(cfgs)
	}
...
func (f *ConfigFinder) UseConfig(name []string, opts ...ConfigOptionFunc) (map[string]*common.Config, error) {
	...
	return f.postProcess(cfg, opts...), nil
}

func (f *ConfigFinder) UseAndSubscribeConfig(name []string, handler common.ConfigChangedHandler, opts ...ConfigOptionFunc) (map[string]*common.Config, error) {
	cfg, err := f.useAndSubscribeConfig(name, handler)
	m := f.config.MeteData
	if err != nil {
		return nil, fmt.Errorf("subscribe config error companion:%s, path:/%s/%s/%s/%s files:%v err:%w", f.config.CompanionUrl, m.Project, m.Group, m.Service, m.Version, name, err)
	}
	return f.postProcess(cfg), nil
}

func (f *ConfigFinder) UseAndSubscribeWithPrefix(prefix string, handler common.ConfigChangedHandler, opts ...ConfigOptionFunc) (map[string]*common.Config, error) {
