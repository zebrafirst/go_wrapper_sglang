...
import (
	...
	companion "git.iflytek.com/AIaaS/finder-go-self/companion"
	"git.iflytek.com/AIaaS/finder-go-self/log"
	"git.iflytek.com/AIaaS/finder-go-self/route"
	"git.iflytek.com/AIaaS/finder-go-self/storage"
	"git.iflytek.com/AIaaS/finder-go-self/utils/fileutil"
	"git.iflytek.com/AIaaS/finder-go-self/utils/serviceutil"
)
...
func (cb *ConfigChangedCallback) Process(path string, node string) {
	...
	if !isSubscribeFile {
		log.Log.Infof("不是订阅的文件，不进行推送：", path, node)
		return
	}
	...
	for {
		...
		if err != nil {
			if err == zk.ErrNoNode {
				log.Log.Infof(" [ Process] %s,%s", path, "get data error,node deleted")
				return
			}
			log.Log.Infof(" [ Process] 从 %s,%s error:%s", path, " 获取数据失败", err)
